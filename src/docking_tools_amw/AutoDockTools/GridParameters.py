...
class GridParameters(UserDict):
    ...
    def read(self, filename):
        ...
        keys = list(self.keys())
        for line in lines:
            words = string.replace(line, '\t', ' ').split()
        ...
        for line in lines:
            ...
            if words!=[] and words[0][0]!='#':
                ...
                #    self['dielectric4']['value'] = self._get_val(values[0])
                    ...
                    print("receptor_types=", string.join(words[1:l]))
                    # TODO: investigate why this is not receptor_types
    ...
    def make_param_string(self, param):
        ...
        if param in self.boolean_param_list:
            ...
            raise NotImplementedError(f"type ({vt.__name__}) of parameter {param} unsupported:\n{self.boolean_param_list}\n{vt}")
    ...
    def set_ligand4(self, ligand_filename, types=None):
        ...
        for i in [' C ']:
            if i not in self['ligand_types']['value']:
                self['ligand_types']['value'] = self['ligand_types']['value'] + ' ' + i
    ...
    def set_receptor4(self, receptor_filename, types=None):
        ...
        print("set_receptor4: self['receptor_types']['value']=", self['receptor_types']['value'])
        for i in ['C']:
            if i not in self['receptor_types']['value']:
                self['receptor_types']['value']
        basename = os.path.basename(receptor_filename)
        self.receptor_filename = basename
        self.receptor_stem = os.path.splitext(basename)[0]
        if receptor_filename!='':
