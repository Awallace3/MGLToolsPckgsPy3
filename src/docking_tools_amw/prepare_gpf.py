...
# convert opt_list if breakdown into main args and parameters
def prepare_gpf(
    receptor_filename=None,
    ligand_filename=None,
    list_filename=None,
    gpf_filename=None,
    output_gpf_filename="receptor.gpf",
    ...
    if (not receptor_filename) or (
        ligand_filename is None and directory is None and ligand_types_defined is False
    ):
    ...
    if npts_increment:
        ...
        if verbose:
            print("before increment npts=", orig_npts)
        for ind in range(3):
            gpfm.gpo["npts"]["value"][ind] += npts_increment
        if verbose:
            print("after increment npts =", gpfm.gpo["npts"]["value"])
...
if __name__ == "__main__":
    ...
    for o, a in opt_list:
        ...
        if o in ("-i", "--i"):
            gpf_filename = a
            if verbose:
                print("reference_gpf_filename=", gpf_filename)
        if o in ("-x", "--x"):
            flexres_filename = a
            if verbose:
        ...
        if o in ("-o", "--o"):
            ...
            if verbose:
                print("output_gpf_filename=", output_gpf_filename)
        if o in ("-p", "--p"):
            parameters.append(a)
            if a.split("=")[0] == "ligand_types":
                ligand_types_defined = True
            if verbose:
        ...
        if o in ("-y", "--y"):
            center_on_ligand = True
            if verbose:
                print("set center_on_ligand to ", center_on_ligand)
        if o in ("-n", "--n"):
            size_box_to_include_ligand = False
            if verbose:
                print("set size_box_to_include_ligand to ", size_box_to_include_ligand)
        if o in ("-I", "--I"):
            npts_increment = int(a)
            if verbose:
                print("set npts_increment to ", npts_increment)
        if o in ("-h", "--"):
            usage()
            sys.exit()
